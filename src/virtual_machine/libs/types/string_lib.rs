...
impl Library for StringLib {
    ...
    fn get_function(&self, name: u64) -> Box<dyn Fn(&mut VM, Vec<Value>) -> Value> {
        match name {
            ...
            x if x == hash_u64!("bytes") => boxed!(Self::bytes),
            x if x == hash_u64!("chars") => boxed!(Self::chars),
            x if x == hash_u64!("split") => boxed!(Self::split),
            x if x == hash_u64!("upper") => boxed!(Self::upper),
            x if x == hash_u64!("lower") => boxed!(Self::lower),
            // Trim
            x if x == hash_u64!("trim") => boxed!(Self::trim),
            x if x == hash_u64!("ltrim") => boxed!(Self::ltrim),
            x if x == hash_u64!("rtrim") => boxed!(Self::rtrim),
            // Search & Replace
            ...
            // Case
            x if x == hash_u64!("title") => boxed!(Self::title),
            // Padding
            x if x == hash_u64!("center") => boxed!(Self::center),
            x if x == hash_u64!("ljust") => boxed!(Self::ljust),
            x if x == hash_u64!("rjust") => boxed!(Self::rjust),
            // Predicates
            x if x == hash_u64!("is_upper") => boxed!(Self::is_upper),
            x if x == hash_u64!("is_lower") => boxed!(Self::is_lower),
            x if x == hash_u64!("is_numeric") => boxed!(Self::is_numeric),
            x if x == hash_u64!("is_alphanumeric") => boxed!(Self::is_alphanumeric),
            x if x == hash_u64!("is_alpha") => boxed!(Self::is_alpha),
            x if x == hash_u64!("is_ascii") => boxed!(Self::is_ascii),
